...
function App() {
  ...
  const sendMessageToElectron = () => {
    if (window.Main) {
      window.Main.sendMessage('Hello from React');
    } else {
    ...
    <div className="flex flex-col">
      ...
      <div className="flex-auto">
        <div className="flex flex-col justify-center items-center h-full pt-32 space-y-4">
          <h1 className="text-2xl dark:text-gray-200">Electron + React + Tailwind with Vite in TS</h1>
          <button
          ...
          {isOpen && (
            <div className="flex flex-col space-y-4 items-center">
              ...
              {fromMain && (
                <div>
                  {' '}
                  <h4 className="dark:text-yellow-200 text-yellow-800">hi from electron</h4>
                </div>
              )}
            </div>
          )}
